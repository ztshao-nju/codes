...
eval_valid_data_path = os.path.join("eval_valid_data")
...
def eval(type, framework, g, args, device, logger, sample_pos_num=None, sample_true_num=None):
    if type == 'train':
        valid_from_file = True
        if valid_from_file:
            logger.info('test: load eval data created from dev_triplets')
            eval_data = read_eval_data(eval_valid_data_path, device)
        else:
            logger.info('test: created eval data from dev_triplets')
        ...
        test_from_file = True
        if test_from_file:
            logger.info('test: load eval data created from test_triplets')
            eval_data = read_eval_data(eval_data_path, device)
        else:
            logger.info('test: create eval data from test_triplets')
...
def create_eval_data(pos_triplets, true_triplets, cnt_e, cnt_r, mode, device, logger, sample_pos_num=None,
                     sample_true_num=None):
...
def create_list_data(pos_triplets, true_triplets, cnt_e, cnt_r, mode, logger, sample_pos_num=None,
                     sample_true_num=None):
    eval_data = []
    ...
    for _index, pos_triplet in enumerate(pos_triplets):
        ...
        if (_index + 1) % 200 == 0:
            logger.info(
                '负样本数据生成进度:{}/{}    时间:{}'.format(_index + 1, len(pos_triplets), time.time() - start))
...
def save_data(eval_data, path, logger):
...
if __name__ == '__main__':
    ...
    if create_test:
        ...
        data_path = eval_valid_data_path
    eval_data = create_list_data(triplets, true_for_test, g.cnt_e, g.cnt_r, 'head', logger, sample_pos_num=None,
                                 sample_true_num=100)
    save_data(eval_data, data_path, logger)
